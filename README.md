...
Для запуска программы перейдите в корневую директорию проекта и выполните комманду:
```
python -m src.main
```
...
1. Выберите источник данных (1, 2 или 3).
2. Введите статус для фильтрации.
3. Ответьте на вопросы о сортировке и фильтрах.
4. Получите результат в консоли.

## Функциональность
* **Чтение данных:** Поддержка загрузки транзакций из файлов форматов JSON, CSV и XLSX.
* **Интерактивное меню:** Удобный интерфейс для выбора режима работы.
* **Фильтрация по статусу:** Отбор операций по статусам (EXECUTED, CANCELED, PENDING) с защитой от некорректного ввода.
* **Сортировка:** Сортировка операций по дате (по возрастанию или убыванию).
* **Фильтрация по валюте:** Возможность отобрать только рублевые транзакции.
* **Поиск по описанию:** Использование регулярных выражений для поиска операций по ключевым словам в описании (без учета регистра).
* **Статистика по категориям:** Подсчет количества операций в заданных категориях с использованием collections.Counter.
* **Красивый вывод:** Форматированный вывод результатов в консоль.

## Технологии

* Python 3.x
* Библиотеки:
* json, csv — для работы с файлами.
